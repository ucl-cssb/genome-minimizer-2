...
- **v0–v4 differ by *loss function*, not hyperparameters** (see
...
| `v4_bayes.yaml` | v4 | bayes (30) | lr, essential-weight, gamma-start, latent-dim, random-state | run → `c4khkttq` |
...
across seeds is the **noise floor** for interpreting the v4 sweep. Result: 2/3
finished, F1 = **0.9862 / 0.9872** → seed noise ≈ **0.001**.

### `v4_bayes.yaml` — Phase C v4 hyperparameter sweep → `c4khkttq`
Bayesian (hyperband early-terminate), 30 runs, on the full v4 loss (recon +
cosine-KL + gene-abundance + essential-gene + L1), hidden-dim 512, 500 epochs.
Sweeps learning-rate (log-uniform 3e-4…3e-3), **essential-weight** {0, 0.1, 0.5,
1, 5}, gamma-start {1, 2}, latent-dim {32, 64}, random-state {12345, 42, 7}.
Results (30/30 finished): best F1 = **0.9885** (`b00w5opr`: essential-weight=0,
gamma=1, latent=32, lr=3e-4, seed=7). **essential-weight=0 wins and ≥1 degrades
F1** (ew=5 → ~0.961) — i.e. the essential-gene loss hurts, which is why the
winner is effectively a tuned v3. Lower LR is better; latent-dim and gamma-start
are ~neutral. Caveat: Bayesian search → unbalanced coverage (19 of 30 runs at
ew=0), so essential-weight is partly confounded with seed/LR.
